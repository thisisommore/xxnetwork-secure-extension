...
// Instead of creating a new channel for each operation
const createChannel = () => {
  // Close any existing channel first
  if (window.xxNetworkChannel) {
    try {
      window.xxNetworkChannel.close();
    } catch (e) {
      console.error("Error closing previous channel:", e);
    }
  }
  
  // Create new channel
  window.xxNetworkChannel = new BroadcastChannel('xx-network-secure-extension');
  return window.xxNetworkChannel;
};

const checkServiceWorkerConnection = async () => {
  try {
    // Try a simple ping message to check if service worker is responsive
    ...
    console.error("Service worker connection failed:", e);
    return false;
  }
};
...
const checkStorageCapacity = async (dataSize) => {
  const quota = await browser.storage.local.getBytesInUse();
  const available = 5242880 - quota; // ~5MB limit in most browsers
